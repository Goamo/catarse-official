...
export function insertPrimitive(type) {
  ...
  const colorInput = document.getElementById("primitiveColor");
  const selectedColor = colorInput?.value || "#0077ff";
  const is2D = [
    "square","rectangle","triangle","equilateralTriangle",
    "circle","ellipse","star","hexagon","semicircle","ring"
  ].includes(type);

  if (is2D) geometry.computeVertexNormals();

  const material = new THREE.MeshStandardMaterial({
    color: selectedColor,
    side: THREE.DoubleSide,
    polygonOffset: is2D,
    polygonOffsetFactor: is2D ? 1 : 0,
    polygonOffsetUnits: is2D ? 1 : 0
  });

  const mesh = new THREE.Mesh(geometry, material);
  // Slight offset in Z to keep 2D shapes on top
  mesh.position.z = is2D ? 0.001 * scene.children.length : 0;
  scene.add(mesh);

  // ——— Add to layerObjects for proper per-layer tracking ———
